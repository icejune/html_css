...
function homeShow(){
	...
	nav_list.click(
		function(){
			...
			//重置用户点击滑动状态
			var siblingsItems = cur_con.siblings();
			siblingsItems.each(function() {
				var _item = $(this),
					_showListItem = _item.find('.show_list');
				if(_showListItem.css('left') !== 'auto' || _showListItem.css('left') !== 0) {
					_showListItem.css({left: 0});
					_item.find('.ico_pre').addClass('disabled_pre').end()
					.find('.ico_next').removeClass('disabled_next');
					//destroyScroll(_item);
					scrollFun(_item);
				}
			});
	...
	function destroyScroll(box) {
		box.find('.ico_pre').unbind('click').end()
		.find('.ico_next').unbind('click');
	}
