...
function bannerSlide() {
	var banner = $('.home .banner'), // 外围banner
		...
		imgArr=['banner_1','banner_2','banner_3','banner_4','banner_5'],
...
function timeLine(){
	...
	init();	
	function init(){
		$('#timeLine .nav li:eq(-4)').addClass('current');
	...
	$('.type_nav li').click(function(){
		$('.type_nav li').removeClass('current');
		$(this).addClass('current');
	})
}
function setCat(cat_id){
	$('#cat_id').val(cat_id);
	getContent();
}
...
function businessList(){
	var li=$('.business_list li')
		close_btn=li.find('.popup .close'),
		popup=li.find('.popup');
	popup.hide();
	$('.business_list li:even').addClass('odd');	
	li.hover(		
		function(){	
			$(this).addClass('hover');
...
$(getEmail);
function getEmail(){
	var btn_get=$('.btn_get_e');
	btn_get.find('a').click(function(e){
			e.preventDefault();
		}
	)
	btn_get.each(
		function(){
			var self=$(this),
				show_address=self.find('.show_address');
			self.hover(
				function(){
					show_address.fadeIn('fast');
				},function(){
					show_address.fadeOut('fast');					
				}
			)
		}
	)
...
$(legalTag);
function legalTag(){
	var title=$('.legal_list li h6');
	$('.legal_list li:first').siblings().children('.context').hide();
	title.click(
		function(){
			var self=$(this),
				show_con=self.siblings('.context');
			show_con.show().parent('li').siblings().children('.context').hide();
		}
	)
}
