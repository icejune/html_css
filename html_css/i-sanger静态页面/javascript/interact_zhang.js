	...
	btn_praise.each(
		function(){
			...
			self.hover(				
				function(){					
					if(self.hasClass("btn_praise_cur")){
						self.text("取消赞").addClass("btn_praise_gray");	
					}
					else{
						self.text("赞+1");
					}
			...
			self.click(
				function(e){
					e.preventDefault();
					self.text(num).addClass("btn_praise_cur");						
					agree.css("display","block");
					if(self.hasClass("btn_praise_gray")){
						self.text(num).removeClass("btn_praise_gray");						
					}
					else{
