...
$(btnPraise);
function btnPraise(){
	var btn_praise=$('.topic_dynamic .btn_praise'),
	...
	btn_praise.each(
		function(){
			...
			self.hover(				
				...
				function(){
					var original_num=self.data('original_num');
					if(self.hasClass('btn_praise_cur')){
						self.text(original_num + 1).removeClass('btn_praise_gray');
					}
					else{
						self.text(original_num);
					}					
				}
			)
			self.click(
				function(e){
					var original_num=self.data('original_num');
					e.preventDefault();
					self.text(original_num + 1).addClass('btn_praise_cur');	
